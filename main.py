...
def apply_move(piles, move):
    pile_idx, remove_count = move # move = ( line , count )
    piles[pile_idx] -= remove_count  # pile_idx = 3 | a = piles[pile_idx]  => a = 5

def minimax(piles, human_turn):
    if is_over(piles):
        return (+1 if human_turn else -1), None
    if human_turn:
        best_score, best_move = +float('inf'), None  # +inf for MIN
    else:  # AI's turn
        best_score, best_move = -float('inf'), None  # -inf for MAX
    ...
    for move in valid_moves(piles):
        ...
        if human_turn:
            ...
            if score > best_score:
                best_score, best_move = score, move
        if best_score == (-1 if human_turn else +1):
            break
...
def main():
    ...
    while running:
        ...
        if (p1_type == 'AI' and current_player == 1) or (p2_type == 'AI' and current_player == 2):
            move = compute_ai_move(piles, DIFFICULTY)
            apply_move(piles, move)
            current_player = 1 if current_player == 2 else 2
            save_unfinished_game(p1_type, p2_type, current_player, piles)
